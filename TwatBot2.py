...
#        print "Connected to",self.hostname
        ...
        if data['user'] in self.factory.admins:
            if data['words'][0] == '^reload':
                try:
                    ...
                    for i in g:
                        if data['words'][1] == i:
                            imp.reload(globals()[data['words'][1]])
            ...
            for i in pluginList:
                if cmd in i.triggers:
                    try:
                        ...
                        self.msg(data['chan'], str(i) +" " +repr(e)[:128])
    ...
    def getDB(self):
        sqlitefname = 'twatbot.db'
        if not os.path.exists(sqlitefname):
            db = sqlite3.connect(sqlitefname)
            db.execute('CREATE TABLE tell (`sender` varchar(32) not null, `to` varchar(32) not null, `message` text not null, `time` datetime not null);')
            db.execute('CREATE TABLE service_nicks (irc_nick varchar(32) not null, service varchar(32) not null, service_nick varchar(32) not null, primary key(irc_nick, service));')
            db.commit()
        else:
            db = sqlite3.connect(sqlitefname)
        return db # MySQLdb.connect (host="localhost",user=keys['mysql_user'],passwd=keys['mysql_pass'],db=keys['mysql_db'])   
...
if __name__ == "__main__":
    ...
    if "#perwl" in chans:
        ...
        reactor.listenTCP(6677,g,interface='127.0.0.1')
    reactor.run()
